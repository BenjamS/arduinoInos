...
//  File file;
  ...
  // Not really necessary as the jpegRender() function defined below seems to automatically take care of positioning
  // based on jpeg image info.
...
//    {
      tft.fillScreen(random(0xFFFF));
      //fileToPlay = random(10); //a random number
      // For random selection of fotos name all files "001.jpg", "002.jpg", etc. and use:
      //(https://stackoverflow.com/questions/73145978/how-can-i-call-random-jpg-from-sd-card-and-display-using-drawsdjpeg)
      char filename [20], temp[20]; // 19 chars max + terminating null
      int i = random(1, 28);
      ...
      if(i < 10){
      sprintf(temp, "/00%s", filename); 
      }
      if(i >= 10 && i < 100){
      sprintf(temp, "/0%s", filename);
      }
      if(i >= 100){
      ...
      //Otherwise the presentation is in alphanumeric order:
